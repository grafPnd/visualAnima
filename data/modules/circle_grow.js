$(window).on('runAnimation', function(e, data){
	if(data.name == 'circle_grow'){
		console.log('circle script');
		var
			w = 0, 
			h = 0,
			op = 1, 
			ml,mt,br;
		data.master.fig[0].appendStyle({background: 'green', opacity: op, width:  w + 'px', height: h + 'px', top: '50%', left: '50%', marginTop: -mt + 'px', marginLeft: -ml + 'px', position: 'absolute'});
